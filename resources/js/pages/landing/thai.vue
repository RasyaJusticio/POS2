<template>
          ...
          <div v-for="item in filteredItems" :key="item.id" class="item-card">
            <div class="card-inner">
              <div class="image-wrapper">
                ...
                />
                <span v-if="item.is_sold_out" class="sold-out-label">Sold Out</span>
              </div>
    ...
    <aside v-if="isCartVisible" class="pos-cart">
      ...
      <button v-if="cart.length > 0" @click="submit(cart)" class="btn btn-success">
        Checkout
      </button>
      <button @click="clearCart" class="btn btn-danger clear-button">
        Clear Cart
      </button>
    </aside>
  ...
  const checkout = () => {
      alert(
          `Total after discount: ${formatCurrency(total.value)} - Proceeding to payment...`
      );
